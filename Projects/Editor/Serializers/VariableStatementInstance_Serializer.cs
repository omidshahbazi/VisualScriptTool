		...
		public override void SerializeInternal(ISerializeData Data, object Instance, System.Type InstanceType, ReferenceList References)
		{
			...
			else
			{
				...
				else
				{
					...
					if (References.Contains(VariableStatementInstance.Statement))
						id = VariableStatementInstance.Statement.ID;
					else
					{
						References.Add(VariableStatementInstance.Statement);
		...
		public override T DeserializeInternal<T>(ISerializeData Data, StatementIDTable StatementIDs, ResolverList Resolvers)
		{
			...
			if (Data is ISerializeArray)
			{
				...
				for (uint i = 0; i < Array.Count; ++i)
				{
					...
					VariableStatementInstanceArray[i] = GetSerializer(targetType).DeserializeInternal<VisualScriptTool.Editor.Language.VariableStatementInstance>(Get<ISerializeObject>(arrayObj, 2), StatementIDs, Resolvers); 
				}
			...
			else
			{
				...
				if (StatementObject != null)
				{
					...
					if (Contains(StatementObjectValue, 1))
					{
						...
						VariableStatementInstance.Statement = StatementSerializer.DeserializeInternal<VisualScriptTool.Language.Statements.Statement>(Get<ISerializeObject>(StatementObjectValue, 2), StatementIDs, Resolvers);
						StatementIDs[VariableStatementInstance.Statement.ID] = VariableStatementInstance.Statement;
					}
					else
					{
						string id = Get<string>(StatementObjectValue, 0);
						Resolvers.Add(new ReferenceResolver(id, VariableStatementInstance, VariableStatementInstance.GetType().GetProperty("Statement", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic)));
					}
				}
				else
				...
				if (PositionObject != null)
				{
					...
					VariableStatementInstance.Position = PositionSerializer.DeserializeInternal<System.Drawing.PointF>(Get<ISerializeObject>(PositionObjectValue, 2), StatementIDs, Resolvers);
				}
				// HeaderSize
				...
				if (HeaderSizeObject != null)
				{
					...
					VariableStatementInstance.HeaderSize = HeaderSizeSerializer.DeserializeInternal<System.Drawing.SizeF>(Get<ISerializeObject>(HeaderSizeObjectValue, 2), StatementIDs, Resolvers);
				}
				// BodySize
				...
				if (BodySizeObject != null)
				{
					...
					VariableStatementInstance.BodySize = BodySizeSerializer.DeserializeInternal<System.Drawing.SizeF>(Get<ISerializeObject>(BodySizeObjectValue, 2), StatementIDs, Resolvers);
				}
				returnValue = (T)(object)VariableStatementInstance;
				VariableStatementInstance.OnPostLoad();
			}
			return returnValue;
		}
