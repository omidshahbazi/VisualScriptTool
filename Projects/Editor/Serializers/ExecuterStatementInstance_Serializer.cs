		...
		public override void SerializeInternal(ISerializeData Data, object Instance, System.Type InstanceType, ReferenceList References)
		{
			...
			else
			{
				...
				else
				{
					...
					if (References.Contains(ExecuterStatementInstance.Statement))
						id = ExecuterStatementInstance.Statement.ID;
					else
					{
						References.Add(ExecuterStatementInstance.Statement);
		...
		public override T DeserializeInternal<T>(ISerializeData Data, StatementIDTable StatementIDs, ResolverList Resolvers)
		{
			...
			if (Data is ISerializeArray)
			{
				...
				for (uint i = 0; i < Array.Count; ++i)
				{
					...
					ExecuterStatementInstanceArray[i] = GetSerializer(targetType).DeserializeInternal<VisualScriptTool.Editor.Language.ExecuterStatementInstance>(Get<ISerializeObject>(arrayObj, 2), StatementIDs, Resolvers); 
				}
			...
			else
			{
				...
				if (StatementObject != null)
				{
					...
					if (Contains(StatementObjectValue, 1))
					{
						...
						ExecuterStatementInstance.Statement = StatementSerializer.DeserializeInternal<VisualScriptTool.Language.Statements.Statement>(Get<ISerializeObject>(StatementObjectValue, 2), StatementIDs, Resolvers);
						StatementIDs[ExecuterStatementInstance.Statement.ID] = ExecuterStatementInstance.Statement;
					}
					else
					{
						string id = Get<string>(StatementObjectValue, 0);
						Resolvers.Add(new ReferenceResolver(id, ExecuterStatementInstance, ExecuterStatementInstance.GetType().GetProperty("Statement", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic)));
					}
				}
				else
				...
				if (PositionObject != null)
				{
					...
					ExecuterStatementInstance.Position = PositionSerializer.DeserializeInternal<System.Drawing.PointF>(Get<ISerializeObject>(PositionObjectValue, 2), StatementIDs, Resolvers);
				}
				// HeaderSize
				...
				if (HeaderSizeObject != null)
				{
					...
					ExecuterStatementInstance.HeaderSize = HeaderSizeSerializer.DeserializeInternal<System.Drawing.SizeF>(Get<ISerializeObject>(HeaderSizeObjectValue, 2), StatementIDs, Resolvers);
				}
				// BodySize
				...
				if (BodySizeObject != null)
				{
					...
					ExecuterStatementInstance.BodySize = BodySizeSerializer.DeserializeInternal<System.Drawing.SizeF>(Get<ISerializeObject>(BodySizeObjectValue, 2), StatementIDs, Resolvers);
				}
				returnValue = (T)(object)ExecuterStatementInstance;
				ExecuterStatementInstance.OnPostLoad();
			}
			return returnValue;
		}
