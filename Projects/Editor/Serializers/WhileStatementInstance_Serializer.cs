		...
		public override void SerializeInternal(ISerializeData Data, object Instance, System.Type InstanceType, ReferenceList References)
		{
			...
			else
			{
				...
				else
				{
					...
					if (References.Contains(WhileStatementInstance.Statement))
						id = WhileStatementInstance.Statement.ID;
					else
					{
						References.Add(WhileStatementInstance.Statement);
		...
		public override T DeserializeInternal<T>(ISerializeData Data, StatementIDTable StatementIDs, ResolverList Resolvers)
		{
			...
			if (Data is ISerializeArray)
			{
				...
				for (uint i = 0; i < Array.Count; ++i)
				{
					...
					WhileStatementInstanceArray[i] = GetSerializer(targetType).DeserializeInternal<VisualScriptTool.Editor.Language.WhileStatementInstance>(Get<ISerializeObject>(arrayObj, 2), StatementIDs, Resolvers); 
				}
			...
			else
			{
				...
				if (StatementObject != null)
				{
					...
					if (Contains(StatementObjectValue, 1))
					{
						...
						WhileStatementInstance.Statement = StatementSerializer.DeserializeInternal<VisualScriptTool.Language.Statements.Statement>(Get<ISerializeObject>(StatementObjectValue, 2), StatementIDs, Resolvers);
						StatementIDs[WhileStatementInstance.Statement.ID] = WhileStatementInstance.Statement;
					}
					else
					{
						string id = Get<string>(StatementObjectValue, 0);
						Resolvers.Add(new ReferenceResolver(id, WhileStatementInstance, WhileStatementInstance.GetType().GetProperty("Statement", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic)));
					}
				}
				else
				...
				if (PositionObject != null)
				{
					...
					WhileStatementInstance.Position = PositionSerializer.DeserializeInternal<System.Drawing.PointF>(Get<ISerializeObject>(PositionObjectValue, 2), StatementIDs, Resolvers);
				}
				// HeaderSize
				...
				if (HeaderSizeObject != null)
				{
					...
					WhileStatementInstance.HeaderSize = HeaderSizeSerializer.DeserializeInternal<System.Drawing.SizeF>(Get<ISerializeObject>(HeaderSizeObjectValue, 2), StatementIDs, Resolvers);
				}
				// BodySize
				...
				if (BodySizeObject != null)
				{
					...
					WhileStatementInstance.BodySize = BodySizeSerializer.DeserializeInternal<System.Drawing.SizeF>(Get<ISerializeObject>(BodySizeObjectValue, 2), StatementIDs, Resolvers);
				}
				returnValue = (T)(object)WhileStatementInstance;
				WhileStatementInstance.OnPostLoad();
			}
			return returnValue;
		}
