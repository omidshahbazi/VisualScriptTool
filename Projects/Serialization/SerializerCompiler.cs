...
namespace VisualScriptTool.Serialization
{
	public sealed class SerializerCompiler
	{
		...
		private const int DATA_STRUCTURE_ID_ID = 0;
		...
		public ISerializationCompileStrategy Strategy
		{
			get;
			set;
		}
		...
		public string Compile(Type Type, string Namespace = "", string ClassName = "")
		{
			...
			if (hasNamespace)
			{
				...
				header.AppendLine("#if !DISABLE_SERIALIZERS");
			...
			serialize.AppendLine("ReferenceList references = new ReferenceList();", indent);
			...
			deserialize.AppendLine("StatementIDTable statementIDs = new StatementIDTable();", indent);
			deserialize.AppendLine("ResolverList resolvers = new ResolverList();", indent);
			deserialize.AppendLine("T returnValue = " + DESERIALIZE_INTERNAL_METHOD_NAME + "<T>(Data, statementIDs, resolvers);", indent);

			deserialize.AppendLine("for (int i = 0; i < resolvers.Count; ++i)", indent);
			deserialize.AppendLine("resolvers[i].Reslve(statementIDs);", ++indent);
			--indent;
			...
			serializeInternal.AppendLine("public override void " + SERIALIZE_INTERNAL_METHOD_NAME + "(ISerializeData Data, object Instance, System.Type InstanceType, ReferenceList References)", indent);
			...
			deserializeInternal.AppendLine("public override T " + DESERIALIZE_INTERNAL_METHOD_NAME + "<T>(ISerializeData Data, StatementIDTable StatementIDs, ResolverList Resolvers)", indent);
			...
			deserializeInternal.AppendLine(typeArrayName + "[i] = GetSerializer(targetType)." + DESERIALIZE_INTERNAL_METHOD_NAME + "<" + GetFullTypeName(Type) + ">(Get<ISerializeObject>(arrayObj, " + DATA_STRUCTURE_VALUE_ID + "), StatementIDs, Resolvers); ", indent);
		...
		private void AppendPrimitive(MemberInfo Member, Type MemberType, int ID, string MemberAccessName, CodeBuilder SerializeMethod, CodeBuilder DeserializeMethod, string ObjectName, ValueType ValueType)
		{
			...
			SerializeMethod.Append(MemberAccessName + ", \"" + Member.Name + "\");");
		...
		private void AppendArray(MemberInfo Member, Type MemberType, int ID, string MemberAccessName, CodeBuilder SerializeMethod, CodeBuilder DeserializeMethod, string ObjectName, ValueType ValueType)
		{
			...
			SerializeMethod.AppendLine("Set<object>(" + ObjectName + ", " + ID + ", null, \"" + Member.Name + "\");", ++indent);
			...
			else if (elementValueType == ValueType.DataStructure)
			{
				...
				SerializeMethod.AppendLine("Set(" + arrayObjectName + ", " + DATA_STRUCTURE_TYPE_ID + ", elementType.AssemblyQualifiedName, \"" + Member.Name + "\");", indent);
		...
		private void AppendDataStructure(MemberInfo Member, Type MemberType, int ID, string MemberAccessName, CodeBuilder SerializeMethod, CodeBuilder DeserializeMethod, string ObjectName)
		{
			...
			if (!MemberType.IsValueType)
			{
				SerializeMethod.AppendLine("string id = string.Empty;", indent);
				SerializeMethod.AppendLine("if (References.Contains(" + MemberAccessName + "))", indent);
				SerializeMethod.AppendLine("id = " + MemberAccessName + ".ID;", ++indent);
				SerializeMethod.AppendLine("else", --indent);
				SerializeMethod.AppendLine("{", indent);
				SerializeMethod.AppendLine("References.Add(" + MemberAccessName + ");", ++indent);
			}
			SerializeMethod.AppendLine("System.Type " + Member.Name + "Type = " + MemberAccessName + ".GetType();", indent);
			SerializeMethod.AppendLine("Set(" + objectName + ", " + DATA_STRUCTURE_TYPE_ID + ", " + Member.Name + "Type.AssemblyQualifiedName, \"" + Member.Name + "\");", indent);
			...
			{
				SerializeMethod.AppendLine("}", --indent);
				SerializeMethod.AppendLine("Set(" + objectName + ", " + DATA_STRUCTURE_ID_ID + ", id, \"" + Member.Name + "\");", indent);
				SerializeMethod.AppendLine("}", --indent);
			}
			...
			DeserializeMethod.AppendLine(MemberAccessName + " = " + serializerName + "." + DESERIALIZE_INTERNAL_METHOD_NAME + "<" + GetFullTypeName(MemberType) + ">(Get<ISerializeObject>(" + objectName + ", " + DATA_STRUCTURE_VALUE_ID + "), StatementIDs, Resolvers);", indent);

			if (!MemberType.IsValueType)
			{
				DeserializeMethod.AppendLine("StatementIDs[" + MemberAccessName + ".ID] = " + MemberAccessName + ";", indent);
				DeserializeMethod.AppendLine("}", --indent);
				DeserializeMethod.AppendLine("else", indent);
				DeserializeMethod.AppendLine("{", indent++);
				string instanceName = GetTypeVariableName(Member.ReflectedType);
				DeserializeMethod.AppendLine("string id = Get<string>(" + objectName + ", " + DATA_STRUCTURE_ID_ID + ");", indent);
				DeserializeMethod.AppendLine("Resolvers.Add(new ReferenceResolver(id, " + instanceName + ", " + instanceName + ".GetType().Get" + (Member is PropertyInfo ? "Property" : "Field") + "(\"" + Member.Name + "\", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic)));", indent);
				DeserializeMethod.AppendLine("}", --indent);
			}
